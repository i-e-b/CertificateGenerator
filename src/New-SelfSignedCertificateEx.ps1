...
.Parameter Authority
	Specifies the path to a public key certificate to use as the issuer of this certificate
.Parameter Password
...
#region Basic Constraints processing
	if ($PSBoundParameters.ContainsKey("IsCA")) {
...
#region Private Key
	...
	if ($Authority) {
		$authPfx = Get-PfxCertificate $Authority
		$sc = New-Object -ComObject X509Enrollment.CSignerCertificate
		$sc.Initialize(0, 0, 4, $authPfx.GetRawCertDataString())
